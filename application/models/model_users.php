...
class Model_Users extends Model
{
    ...
    public function CreateTreeDepartments($DeptList, $ParentID = 0){
        if(!isset($DeptList[$ParentID])) {
            return FALSE;
        }

        $tree = '<ul>';
        foreach ($DeptList[$ParentID] as $id => $Dept) {
            $tree .= '<li><a href="?users/departments/'.$Dept['DEPT_ID'].'">'.$Dept['DEPT_NAME'].'</a>';
            $tree .= $this->CreateTreeDepartments($DeptList, $Dept['DEPT_ID']);
            $tree .= '</li>';
        }
        $tree .= '</ul>';
        return $tree;
